    ...
    <div>
        <v-row class="d-flex justify-center">
            ...
            <v-card width="100%" v-show="state.isAuthenticated" elevation="0">
                ...
                ><div data-plyr-provider="youtube" data-plyr-embed-id="inwDpNFXfiA"></div>
                </vue-plyr>
            </v-card>
                </v-row>
            </v-col>
        ...
        <v-row class="d-flex justify-center">
          <v-card width="90%">
          <iframe :src="aula.pdfUrl" width="100%" height="600px"></iframe>
          </v-card>
        </v-row>              
    </div>
</template>
...
//   },
  methods: {
    playerReady () {
      this.player.currentTime = Math.round(this.aula.progress / 100 * 6261);
    },
    setProgress() {
        this.$store.dispatch("setProgressBar", {
            progress: Math.round(this.player.currentTime / 6261 * 100),
            course: "BasicSemana",
            aula: "aula1"
        })
    }
