    ...
    <div>
        <v-row class="d-flex justify-center">
            ...
            <v-card width="100%" v-show="state.isAuthenticated" elevation="0">
                ...
                ><div data-plyr-provider="youtube" data-plyr-embed-id="_CBsT9xVOOA"></div>
                </vue-plyr>
            </v-card>
                </v-row>
            </v-col>
...
//   },
  methods: {
    playerReady () {
      this.player.currentTime = Math.round(this.aula.progress / 100 * 7170);
    },
    setProgress() {
        this.$store.dispatch("setProgressBar", {
            progress: Math.round(this.player.currentTime / 7170 * 100),
            course: "BasicSemana",
            aula: "aula8"
        })
    }
