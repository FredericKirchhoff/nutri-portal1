        ...
        <v-row class="d-flex justify-center">
            ...
            <v-col cols="12" md="10">
                <v-row class="d-flex justify-center black--text text-h5">
                   Prescrição de Probióticos e Parabióticos na Imunidade, <br/>
                   Constipação e Alterações Comportamentais
                </v-row>
            ...
            <v-card width="100%" v-show="state.isAuthenticated" elevation="0">
                ...
                ><div data-plyr-provider="youtube" data-plyr-embed-id="zVZk-eNueck"></div>
                </vue-plyr>
            </v-card>
                </v-row>
            </v-col>
...
//   },
  methods: {
    playerReady () {
      this.player.currentTime = Math.round(this.aula.progress / 100 * 6050);
    },
    setProgress() {
        this.$store.dispatch("setProgressBar", {
            progress: Math.round(this.player.currentTime / 6050 * 100),
            course: "BasicSemana",
            aula: "aula5"
        })
    }
