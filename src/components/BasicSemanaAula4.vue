    ...
    <div>
        <v-row class="d-flex justify-center">
            ...
            <v-card width="100%" v-show="state.isAuthenticated" elevation="0">
                ...
                ><div data-plyr-provider="youtube" data-plyr-embed-id="XCtZfNJZJqU"></div>
                </vue-plyr>
            </v-card>
                </v-row>
            </v-col>
        ...
        <v-row class="d-flex justify-center black--text text-h5 my-12">
            Materiais da aula: 1. Nutrigenética
        </v-row>
        <v-row class="d-flex justify-center">
          <v-card width="90%">
          <iframe :src="aula.pdfUrl_et" width="100%" height="800px"></iframe>
          </v-card>
        </v-row>
        <v-divider></v-divider>
        <v-row class="d-flex justify-center black--text text-h5 my-12">
            Materiais da aula: 2. Nutrigenomica
        </v-row>
        <v-row class="d-flex justify-center">
          <v-card width="90%">
          <iframe :src="aula.pdfUrl_om" width="100%" height="800px"></iframe>
          </v-card>
        </v-row>              
    </div>
</template>
...
//   },
  methods: {
    playerReady () {
      this.player.currentTime = Math.round(this.aula.progress / 100 * 6795);
    },
    setProgress() {
        this.$store.dispatch("setProgressBar", {
            progress: Math.round(this.player.currentTime / 6795 * 100),
            course: "BasicSemana",
            aula: "aula4"
        })
    }
